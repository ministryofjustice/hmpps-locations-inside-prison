...
context('View Locations Show', () => {
  ...
  function validateBreadcrumbs(viewLocationsShowPage: ViewLocationsShowPage, locationHierarchy: LocationSummary[]) {
    ...
    viewLocationsShowPage.breadcrumbs().eq(1).contains('Locations').should('have.attr', 'href').and('equal', '/TST')
  ...
  function setUpCommonStubs(roles: string[] = []) {
    ...
    LocationsApiStubber.stub.stubGetPrisonConfiguration({
      prisonId: 'TST',
      certificationActive: isCertActive ? 'ACTIVE' : 'INACTIVE',
    })
  }
  ...
  function testShow({
    ...
    viewLocationsShowPage.certifiedTag().should(`${location.leafLevel && location.certifiedCell ? '' : 'not.'}exist`)
    const statusTag =
    ...
    if (location.status === 'INACTIVE') {
      ...
      if (isCertActive) {
        ...
        viewLocationsShowPage
          ...
          .eq(3)
          .find('.govuk-summary-list__key')
          .contains('Certified working capacity')
        viewLocationsShowPage
          .inactiveBannerRows()
          .eq(3)
          .find('.govuk-summary-list__value')
    ...
    if (location.status === 'DRAFT') {
      ...
      viewLocationsShowPage
        ...
        .should(
          'have.attr',
          'href',
          `/TST/cell-certificate/change-requests/${location.pendingApprovalRequestId}${roles.includes('RESI__CERT_REVIEWER') ? '/review' : ''}`,
        )
    ...
    if (location.status !== 'NON_RESIDENTIAL') {
      ...
      viewLocationsShowPage.locationDetailsRows().eq(detailsRows).find('.govuk-summary-list__key').contains('Used for')
      viewLocationsShowPage
        .locationDetailsRows()
        .eq(detailsRows)
        .find('.govuk-summary-list__value')
      ...
      viewLocationsShowPage.locationDetailsRows().eq(detailsRows).find('.govuk-summary-list__value').contains('Office')
      detailsRows += 1
    }
  ...
  context('With certification active', () => {
    beforeEach(() => {
      isCertActive = true
    })
    ...
    context('With the default role', () => {
      ...
      context('When the location is a Wing', () => {
        const locationDetails: Partial<Location> = {
          ...
          sortName: 'A',
          isResidential: true,
          capacity: { maxCapacity: 100, workingCapacity: 94, certifiedNormalAccommodation: 94 },
      ...
      context('When the location is a Landing', () => {
        const locationDetails: Partial<Location> = {
          ...
          sortName: 'A-1',
          isResidential: true,
          capacity: { maxCapacity: 20, workingCapacity: 14, certifiedNormalAccommodation: 14 },
          numberOfCellLocations: 4,
        }
        const locationHierarchy = [
          {
      ...
      context('When the location is a Cell', () => {
        const locationDetails: Partial<Location> = {
          ...
          sortName: 'A-1-001',
          isResidential: true,
          capacity: { maxCapacity: 2, workingCapacity: 1, certifiedNormalAccommodation: 1 },
          numberOfCellLocations: 1,
        }
        const locationHierarchy = [
          {
    ...
    context('With the RESI__CERT_REVIEWER role', () => {
      beforeEach(() => {
        setUpCommonStubs(['RESI__CERT_REVIEWER'])
      })
      ...
      context('When the location is a Wing', () => {
        const locationDetails: Partial<Location> = {
          ...
          sortName: 'A',
          isResidential: true,
          capacity: { maxCapacity: 100, workingCapacity: 94, certifiedNormalAccommodation: 1 },
          numberOfCellLocations: 4,
        }
        const locationHierarchy = [
          {
        ...
        context('When the location is Locked Draft', () => {
          ...
          it('Correctly presents the API data', () => {
            testShow({ location, locationHierarchy, roles: ['RESI__CERT_REVIEWER'] })
          })
        })
      })
    })
  ...
  context('With certification inactive', () => {
    beforeEach(() => {
      isCertActive = false
    })
        ...
        context('When the location is Inactive', () => {
          beforeEach(() => {
            location = LocationFactory.build({
              ...locationDetails,
              status: 'INACTIVE',
              lastDeactivationReasonForChange: 'Test reason',
            })
