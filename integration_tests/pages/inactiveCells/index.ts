...
export default class InactiveCellsIndexPage extends Page {
  ...
  locationsTable = (inactiveStatus?: string): PageElement =>
    cy.get(`[data-qa=inactive-cells-table${inactiveStatus ? `-${inactiveStatus}` : ''}]`)

  locationsTableRows = (inactiveStatus?: string): PageElement => this.locationsTable(inactiveStatus).find('tbody tr')
  ...
  locationsTableCells = (row: PageElement) => {
    ...
    return {
      ...
      changeType: children.eq(1),
      estimatedReactivationDate: children.eq(2),
      requestedBy: children.eq(2),
