...
describe('populateDecoratedResidentialSummary - Link Visibility Functions', () => {
  ...
  describe('Summary Cards Generation (via middleware)', () => {
    // Summary card generation logic is inherently complex as it depends on:
    // 1. Async calls to locationsService (getResidentialSummary, getPrisonConfiguration, etc.)
    // 2. Decorating locations, which requires multiple service lookups
    // 3. Complex conditional logic based on location status, certification requirements, etc.
    //
    // The core card generation logic is thoroughly tested through the link visibility
    // functions above (showChangeCapacityLink, showSanitationChangeLink, etc.), which
    // are called directly in the middleware to determine what links to include on cards.
    //
    // Integration tests for this middleware are better suited for end-to-end testing
    // through the controller/route layer where all service mocks can be properly set up.
    it('placeholder: card generation logic is tested via link visibility functions', () => {
