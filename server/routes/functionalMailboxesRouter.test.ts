...
describe('functional mailbox routes', () => {
  ...
  it('pre-populates the default mailbox edit form', async () => {
    locationsService.getNotificationMailbox.mockResolvedValue(mailbox('CERT_VIEWER', ['viewer@example.com']))

    const response = await request(app).get('/functional-mailboxes/default/CERT_VIEWER/edit').expect(200)

    expect(response.text).toContain('Edit default mailbox')
    expect(response.text).toContain('Certification viewer')
    expect(response.text).toContain('viewer@example.com')
    expect(locationsService.getNotificationMailbox).toHaveBeenCalledWith(undefined, 'CERT_VIEWER')
  })

  it('replaces an existing default mailbox through the edit journey', async () => {
    locationsService.getNotificationMailbox.mockResolvedValue(mailbox('CERT_VIEWER', ['old@example.com']))

    await request(app)
      .post('/functional-mailboxes/default/CERT_VIEWER/edit')
      .type('form')
      .send({ emailAddresses: 'New@Example.com\nsecond@example.com' })
      .expect(302)
      .expect('Location', '/functional-mailboxes')

    expect(locationsService.getNotificationMailbox).toHaveBeenCalledWith(undefined, 'CERT_VIEWER', undefined)
    expect(locationsService.replaceNotificationMailbox).toHaveBeenCalledWith(
      undefined,
      'CERT_VIEWER',
  ...
  it('returns not found when editing a missing default mailbox', async () => {
    const response = await request(app).get('/functional-mailboxes/default/CERT_VIEWER/edit').expect(404)

    expect(response.text).toContain('Page not found')
  })

  it('returns not found when posting an edit for a missing default mailbox', async () => {
    const response = await request(app)
      .post('/functional-mailboxes/default/CERT_VIEWER/edit')
      .type('form')
      .send({ emailAddresses: 'viewer@example.com' })
      .expect(404)
